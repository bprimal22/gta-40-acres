...
import rawPlan from '../../public/data/welch-east-yard-plan.json' with { type: 'json' };

type Volume = { planes: T.Plane[]; bounds: T.Box3 };
type Face = [T.Vector3, T.Vector3, T.Vector3];
export interface WelchEastYardOptions {
 ...
 */
export function buildWelchEastYard(options: WelchEastYardOptions) {
  ...
  const endHeight = (which: number, t: number) => {
    ...
    // Missing northern inner ground has one explicit known LOW source datum,
    // recorded in plan. Interpolate only to the first actual authored support.
  ...
  const nominal = (s: number, t: number, width: number, edgeY: number) => {
    const d = t * width;
    let y = T.MathUtils.lerp(plan.wallHeight, edgeY, T.MathUtils.clamp((d - plan.innerWalkWidth) / (width - plan.innerWalkWidth), 0, 1));
  ...
  const rows = plan.rows.map(r => {
    ...
    const ts = [...new Set([0, plan.innerWalkWidth / r.width, 1, ...(dense ? plan.fractions : [])])].sort((a, b) => a - b);
  ...
  for (let i = 0; i < rows.length - 1; i++) {
    ...
    while (ai < a.length - 1 || bi < b.length - 1) {
      ...
      if (an && bn && Math.abs(an.t - bn.t) < .001) { top(a[ai].p, b[bi].p, an.p, faces); top(an.p, b[bi].p, bn.p, faces); ai++; bi++; }
  ...
  const meshes = positions.map((values, i) => {
    ...
    const m = new T.Mesh(g, i === 0 ? options.concrete : options.grass); m.name = i === 0 ? 'Welch east yard: connected walks' : 'Welch east yard: graded planting'; m.receiveShadow = true; m.castShadow = true; return m;
  });
  ...
  return { meshes, materials: [] as T.MeshStandardMaterial[], colliderGeometries: meshes.map(m => m.geometry), volumes: sourceClearanceVolumes, sourceClearanceVolumes, authoredGroundTrimVolumes, oldGroundTrimVolumes: authoredGroundTrimVolumes, height, inside, contains: inside, paved, local,
    ...
    stats: { scope: 'Welch east graded forecourt, exact outer gray Speedway edge', wallHeight: plan.wallHeight, areaM2: area, rowCount: rows.length, topTriangles: topFaces.length, triangles: meshes.reduce((n, m) => n + m.geometry.attributes.position.count / 3, 0), materialBatches: meshes.length, ownedMaterials: 0, sourceCutVolumes: sourceClearanceVolumes.length, oldGroundTrimVolumes: authoredGroundTrimVolumes.length, farEdgeExact: true, northernInnerJoinUsesExplicitLowDatum: 5.62 },
