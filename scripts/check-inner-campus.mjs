...
const treePlacements=walkway.treePlacements.filter(p=>p.x<-200&&p.x>-385&&p.z<55&&p.z>-100);
...
const plan=JSON.parse(readFileSync('public/data/inner-campus-plan.json'));
const get=id=>plan.routes.find(r=>r.wayId===id).points;
const north=get(15387933),south=get(31954662),foot=get(126328799),branch=get(130309571);
const union=[...north,...branch.slice(1)];
const mall=[...north.slice(0,4),...south.slice(1),...foot.slice(1),[-275,38.7]];
check('Mapped Inner Campus lanes have matching floor at center and both edges',()=>{
 let samples=0,maxError=0;for(const r of plan.routes)for(let i=1;i<r.points.length;i++){
  const a=r.points[i-1],b=r.points[i],dx=b[0]-a[0],dz=b[1]-a[1],len=Math.hypot(dx,dz),count=Math.ceil(len/1.5);
  for(let j=0;j<count;j++)for(const off of [-1.4,0,1.4]){const t=(j+.2)/count,h=ground(a[0]+dx*t-dz/len*off,a[1]+dz*t+dx/len*off);samples++;maxError=Math.max(maxError,h.error);}
 }return{samples,maxError};
});
check('Normal 8 m/s controller reaches Union court and returns',()=>walk([...union,...union.slice(0,-1).reverse()],8));
check('Normal controller connects Tower north to West Mall and returns',()=>walk([...mall,...mall.slice(0,-1).reverse()],8));
check('Both Tower west sidewalks remain traversable',()=>{
 const runs=[];for(const side of [-1,1]){
  const route=south.map((p,i)=>{const a=south[Math.max(0,i-1)],b=south[Math.min(south.length-1,i+1)],dx=b[0]-a[0],dz=b[1]-a[1],len=Math.hypot(dx,dz);return[p[0]-dz/len*4.8*side,p[1]+dx/len*4.8*side];});
  runs.push(walk([...route,...route.slice(0,-1).reverse()],8));
 }return runs;
});
check('Union south apron connects the existing mall to the corrected entrance',()=>{
 const center=new T.Vector2(-393.20334,8.81191),east=new T.Vector2(.9961722,.0874122),out=new T.Vector2(-east.y,east.x);
 const p=(u,d)=>center.clone().addScaledVector(east,u).addScaledVector(out,d).toArray();
 return [walk([p(0,7),p(0,5),p(0,3),p(0,.55),p(0,3),p(0,7)],8),walk([p(-6.5,4),p(0,4),p(6.5,4),p(0,4),p(-6.5,4)],1.65)];
});
walkway.dispose(world);world.free();
const result={passed:!failures.length,checks,failures,trees:treePlacements.length,limits:'Actual authored scene and tree wood colliders; no streamed tiles or browser controls. Actual play and visual acceptance remain required.'};
writeFileSync(process.env.INNER_CAMPUS_EVIDENCE??'/tmp/ut-iteration-46/inner-campus-check.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));if(failures.length)process.exitCode=1;
